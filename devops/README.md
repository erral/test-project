...
DEPLOY_ENV=prod
DEPLOY_HOST=test-project.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
DOCKER_CONFIG=.docker
STACK_NAME=test-project-example-com
```
...
You need either a Ubuntu or Debian based system for test-project.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    test-project.example.com:
      ansible_user: root
      host: test-project
      hostname: test-project.example.com
```
...
Deploy the stack defined in `devops/stacks/test-project.example.com.yml` to the remote server with:
